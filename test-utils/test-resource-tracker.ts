...
// Use a unique identifier to avoid collision with Node's built-in __dirname when Jest transpiles to CommonJS
...
export class TestResourceTracker {
  ...
  private trackingFile = join(TEST_UTILS_DIR, "../../.test-resources.json");
  ...
  async cleanup(googleToken: string, microsoftToken: string) {
    await this.load();
    console.log(
      `[CLEANUP] Found ${this.resources.length} resources to clean up`
    );
    ...
    for (const resource of this.resources.reverse()) {
      try {
        ...
        console.log(
          `[CLEANUP] \u2713 Deleted ${resource.type}: ${resource.id}`
        );
      } catch (error) {
        console.error(
          `[CLEANUP] \u2717 Failed to delete ${resource.type}: ${resource.id}`,
          error
        );
      }
    }
